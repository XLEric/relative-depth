...
class DataLoader(object):
	...
	def parse_DIW_csv(self, _filename):
		...
		while _line_idx < _n_lines:
			...
			# print(_handle[_sample_idx]['img_filename'])
	...
	def load_indices(self, depth_indices):
		...
		loader = transforms.Compose([
			transforms.Scale((g_input_height,g_input_width)),
			transforms.ToTensor(),
		...
		for i in range(0,n_depth):
			...
			# img_name = './DIW_train_val/687d836eb6178e49d1d71ae9124184818b5bbd69.thumb'
			n_point = self.relative_depth_handle[chosen_idx]['n_point']
			# print('img_nameimg_nameimg_nameimg_nameimg_nameimg_name',img_name)
			...
			if img.size()[0] == 1:
				...
				color[i,0,:,:].copy_(img[0])
				color[i,1,:,:].copy_(img[0])
				color[i,2,:,:].copy_(img[0])
			else:
				color[i,:,:,:].copy_(img)
	...
	def load_next_batch(self, batch_size):
		...
		# print('depth_indices :',depth_indices,len(depth_indices))

		# depth_indices=[depth_indices[i].cpu().numpy() for i in range(batch_size)]

		depth_indices = depth_indices.cpu().numpy()
		# print('depth_indices 2 : ',len())
